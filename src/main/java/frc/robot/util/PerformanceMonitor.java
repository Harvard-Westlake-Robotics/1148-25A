...
/**
 * Advanced performance monitoring utility for optimizing robot periodic loop
 * times. Tracks
 * execution times, identifies bottlenecks, and provides adaptive performance
 * management.
 */
public class PerformanceMonitor {
    ...
    private void recordPerformanceData(String subsystemName, double executionTimeMs) {
        SubsystemPerformanceData data = subsystemMetrics.computeIfAbsent(subsystemName,
                k -> new SubsystemPerformanceData());
    ...
    private void updateGlobalMetrics() {
        ...
        double totalExecutionTime = subsystemMetrics.values().stream()
                .mapToDouble(SubsystemPerformanceData::getLatestTime)
                .sum();

        averageLoopTime = subsystemMetrics.values().stream()
                .mapToDouble(SubsystemPerformanceData::getAverageTime)
                .sum();

        // Check if we should enter/exit degraded mode
        boolean shouldEnterDegradedMode = averageLoopTime > DEGRADED_MODE_THRESHOLD_MS || consecutiveSlowLoops > 5;
    ...
    private boolean shouldLogPerformanceData() {
        ...
        return Timer.getFPGATimestamp() % (TARGET_LOOP_TIME_MS / 1000.0 * 4) < 0.001; // Log every 4th loop
    }
    ...
    /**
     * Determines if a subsystem should skip non-critical operations to improve
     * performance.
     *
     * @param subsystemName The name of the subsystem
    ...
    /**
     * Gets the recommended update frequency for a subsystem based on its
     * performance.
     *
     * @param subsystemName      The name of the subsystem
